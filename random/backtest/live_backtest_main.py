...
try:
    ...
    from . import live_backtest_feature_eng as live_feature_eng
    from . import live_backtest_engine as live_engine
...
# --- Main Logging Setup for this script ---
...
# Optional: File Handler for main script's log
# log_file = live_cfg.LOG_DIR / f"live_backtest_main_{time.strftime('%Y%m%d_%H%M%S')}.log"
...
def main():
    ...
    # 3. Feature Engineering for all markets
    all_markets_features_list = []
    live_data_utils.clear_live_data_caches() # Clear caches before new run
    ...
    for index, market_row in live_outcomes_df.iterrows():
        ...
        try:
            market_features_df = live_feature_eng.generate_features_for_market_live(
                market_row,
                model_feature_names
            )
            if not market_features_df.empty:
                all_markets_features_list.append(market_features_df)
                main_logger.info(f"  Generated {len(market_features_df)} decision points for {market_ticker}.")
            else:
                main_logger.warning(f"  No decision points generated for {market_ticker}.")
        except Exception as e:
            main_logger.error(f"Error generating features for {market_ticker}: {e}", exc_info=True)
    ...
    main_logger.info(f"Total decision points across all live markets: {len(all_features_to_backtest_df)}")

    # Quick check for NaNs in crucial columns before passing to engine
    cols_to_check = ['market_ticker', 'decision_timestamp_s', 'resolution_time_ts', 'target', 'strike_price', 'time_to_resolution_minutes']
    for col in model_feature_names: # also check all model features
    ...
    if nan_check.sum() > 0:
        ...
        # Drop rows where any of the *model_feature_names* are NaN, as scaler/model can't handle them
        ...
        main_logger.info("No NaNs found in essential/model columns of combined feature DataFrame.")
    ...
    if not all_features_to_backtest_df.empty:
        ...
        try:
            total_pnl_cents, total_trades, trades_log_df = live_engine.run_live_data_backtest(
                all_features_to_backtest_df,
                model,
                scaler,
                model_feature_names
            )
            ...
            main_logger.info(f"Overall P&L from live backtest engine: {total_pnl_cents / 100.0 :.2f} USD")
            main_logger.info(f"Total trades made by engine: {total_trades}")
            main_logger.info(f"Daily trade logs are in: {live_cfg.LOG_DIR}")

            if not trades_log_df.empty:
                # Save consolidated log
                consolidated_log_path = live_cfg.LOG_DIR / f"all_live_trades_summary_{time.strftime('%Y%m%d_%H%M%S')}.csv"
                ...
                main_logger.info("No trades were made, so no consolidated log to save.")

        except Exception as e:
            main_logger.error(f"An error occurred during run_live_data_backtest: {e}", exc_info=True)
    else:
        main_logger.error("Feature DataFrame is empty. Cannot run live backtest engine.")
