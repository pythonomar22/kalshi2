...
def main():
    ...
    for index, market_row in live_outcomes_df.iterrows():
        ...
        try:
            ...
            main_logger.error(f"Error generating features for {market_ticker}: {e}", exc_info=False) # Set exc_info to False for less verbose logs during loop
    ...
    if not all_features_to_backtest_df.empty:
        ...
        try:
            # Engine now returns final_capital as well
            simple_sum_pnl_cents, total_contracts_traded, trades_log_df, final_capital_cents = live_engine.run_live_data_backtest(
