...
try:
    ...
    elif script_path.parent.parent.name == "notebooks":
         BASE_PROJECT_DIR = script_path.parent.parent.parent
    else: 
        cwd = Path.cwd()
        if (cwd / "notebooks").exists(): BASE_PROJECT_DIR = cwd
        elif cwd.name == "notebooks": BASE_PROJECT_DIR = cwd.parent
        else: BASE_PROJECT_DIR = cwd 
    logger_early_setup.info(f"Determined BASE_PROJECT_DIR: {BASE_PROJECT_DIR.resolve()}")
except Exception as path_e:
    logger_early_setup.error(f"Error determining BASE_PROJECT_DIR: {path_e}. Defaulting to CWD.")
...
LOGS_DIR = NOTEBOOKS_DIR / "logs" / "classifier_kelly_backtests" # Updated logs subdir
MODEL_ARTIFACTS_ROOT_DIR = NOTEBOOKS_DIR / "trained_models" 
CLASSIFIER_MODEL_SUBDIR = "logreg" 
MODEL_ARTIFACTS_DIR = MODEL_ARTIFACTS_ROOT_DIR / CLASSIFIER_MODEL_SUBDIR
...
logger = logging.getLogger("ClassifierKellyBacktest") # Updated logger name
...
logger.info(f"Classifier Model Artifacts Dir: {MODEL_ARTIFACTS_DIR.resolve()}")
logger.info(f"Backtest logs will be saved under: {LOGS_DIR.resolve()}")

# --- Import project modules ---
import utils 
import sizing # Now uses Kelly
import logreg_strategy_v1 as strategy 

if not strategy.load_model_artifacts(MODEL_ARTIFACTS_DIR): # Pass the correct model dir
    logger.critical("CRITICAL: Failed to load classifier model artifacts in strategy module. Aborting.")
    exit()
...
try:
    ...
    if not outcomes_files:
        raise FileNotFoundError(f"No NTM outcomes CSV found in {KALSHI_DATA_ROOT_DIR.resolve()}")
    ...
    logger.critical(f"Backtest: CRITICAL - {e}. Market outcomes file is required.")
...
BACKTEST_START_DATE_STR = "2025-05-11" 
BACKTEST_END_DATE_STR = "2025-05-11"   
...
# --- Sizing Configuration (passed to sizing.py or used as overrides) ---
sizing.KELLY_FRACTION = 0.1 # Global setting in sizing.py
sizing.MAX_CAPITAL_ALLOCATION_PERCENTAGE_OF_TOTAL = 0.10 # Global setting
sizing.MAX_CONTRACTS_PER_TRADE = 200 # Global setting

# Strategy decision thresholds (can also be set in strategy file)
strategy.MIN_MODEL_PROB_FOR_CONSIDERATION = 0.80 # Must be >55% confident
strategy.EDGE_THRESHOLD_FOR_TRADE = 0.05      # Must have a 5% edge over Kalshi implied odds


def run_hourly_session_backtest(session_markets_df, binance_session_features_df, global_config_state):
    """
    Runs backtest for a single hourly session.
    global_config_state is a dictionary to pass and update current_capital_usd.
    """
    ...
    session_trades_log = []
    # Capital is managed globally but PNL/contracts are tracked per session for reporting
    session_pnl_cents = 0
    session_contracts_traded = 0
    ...
    for idx, market_row in session_markets_df.iterrows():
        ...
        logger.debug(f"  Processing Market: {market_ticker}, Strike: {kalshi_strike_price}, Closes: {market_close_dt_utc.isoformat()}")

        parsed_ticker_info = utils.parse_kalshi_ticker_info(market_ticker)
        if not parsed_ticker_info:
            logger.warning(f"    Could not parse ticker {market_ticker} for Kalshi data path. Skipping.")
            continue
        ...
        while current_decision_dt_utc <= latest_permissible_decision_dt_utc:
            signal_ts_utc = int(current_decision_dt_utc.timestamp()) 
            features_for_ts_utc = signal_ts_utc - 60 

            if features_for_ts_utc not in binance_session_features_df.index:
                current_decision_dt_utc += timedelta(minutes=global_config_state['decision_interval_minutes'])
                continue
            
            btc_history_for_features = binance_session_features_df[
                binance_session_features_df.index <= features_for_ts_utc
            ].copy()
            ...
            current_kalshi_prices = utils.get_kalshi_prices_at_decision(
                kalshi_market_day_data_df, signal_ts_utc, 
            ...
            feature_vector_series = strategy.generate_live_features(
                ...
                current_kalshi_bid=current_yes_bid, current_kalshi_ask=current_yes_ask,
                kalshi_market_history_df=kalshi_market_day_data_df,
            ...
            # get_trade_decision now returns (trade_action, model_prob_for_chosen_side, entry_price_cents_for_chosen_side)
            ...
            if trade_action and pd.notna(model_prob_chosen_side) and pd.notna(entry_price_chosen_side):
                num_contracts = sizing.calculate_kelly_position_size(
                    ...
                    # Can add overrides for kelly_fraction or max_alloc_pct here if needed
                )
                ...
                if num_contracts > 0:
                    ...
                    global_config_state['current_capital_usd'] += (pnl_this_trade_cents / 100.0) # Update global capital
                    session_pnl_cents += pnl_this_trade_cents
                    session_contracts_traded += num_contracts
                    
                    log_msg = (f"TRADE: {market_ticker} @ {current_decision_dt_utc.strftime('%H:%M')} | P(YES):{predicted_proba_yes:.3f} "
                               f"P({trade_action.split('_')[1]}):{model_prob_chosen_side:.3f} | "
                    ...
                    logger.info(log_msg)
                else: # Kelly sizing resulted in 0 contracts
                    trade_action = "NO_TRADE" # Ensure it's marked as no trade
            
            if not trade_action or num_contracts == 0 : # If no trade was signaled or sized to 0
            ...
            session_trades_log.append({
                ...
                "kalshi_yes_bid_at_decision": current_yes_bid,
                "kalshi_yes_ask_at_decision": current_yes_ask,
                ...
                "pnl_cents": pnl_this_trade_cents,
...
def main_backtest_loop():
    if not MARKET_OUTCOMES_CSV_PATH or not MARKET_OUTCOMES_CSV_PATH.exists(): # Guard already at top
        logger.critical("Market outcomes CSV not found. Aborting.")
        return
    ...
    if df_all_market_details.empty:
        logger.warning(f"No markets found for overall backtest period: {BACKTEST_START_DATE_STR} to {BACKTEST_END_DATE_STR}")
        return

    # Use the get_session_key_from_market_row from utils.py
    ...
    logger.info(f"Backtest: Identified {len(grouped_sessions)} unique hourly sessions to process.")

    global_config_state = {
        'initial_capital_usd': INITIAL_CAPITAL_USD,
        'current_capital_usd': INITIAL_CAPITAL_USD, # This will be updated
        'trade_decision_offset_minutes_from_market_close': TRADE_DECISION_OFFSET_MINUTES_FROM_MARKET_CLOSE,
        'decision_interval_minutes': DECISION_INTERVAL_MINUTES,
        'kalshi_max_staleness_seconds': KALSHI_MAX_STALENESS_SECONDS
    }
    overall_trades_log = []
    overall_pnl_cents = 0
    overall_contracts_traded = 0
    ...
    # TA parameters in utils are now the single source. Strategy imports them.

    # Sort sessions chronologically for processing
    sorted_session_keys = sorted(grouped_sessions.groups.keys(), key=lambda x: (utils.parse_kalshi_ticker_info(f"DUMMY-{x.replace('_', '')}-T1")['event_resolution_dt_utc']))


    for session_key in sorted_session_keys:
        session_markets_df = grouped_sessions.get_group(session_key)
        
        session_log_file_path = LOGS_DIR / f"backtest_LogRegKelly_{session_key}_{run_timestamp}.txt"
        current_file_handlers = [h for h in logging.root.handlers if isinstance(h, logging.FileHandler)]
        for h_file in current_file_handlers: h_file.close(); logging.root.removeHandler(h_file)
            
        file_handler = logging.FileHandler(str(session_log_file_path))
        ...
        binance_hist_start_dt = min_market_open_dt_session - timedelta(minutes=max_ta_lookback_minutes + 60) 
        binance_hist_end_dt = max_market_close_dt_session + timedelta(minutes=60) 

        logger.info(f"  Session {session_key}: Pre-loading Binance data from {binance_hist_start_dt.strftime('%Y-%m-%d %H:%M')} to {binance_hist_end_dt.strftime('%Y-%m-%d %H:%M')}")
        
        binance_session_features_df = utils.load_and_prepare_binance_range_with_features(
            binance_hist_start_dt, binance_hist_end_dt
        )
        if binance_session_features_df is None or binance_session_features_df.empty:
            logger.critical(f"  Failed to load Binance history for session {session_key}. Skipping session.")
            if file_handler: logging.root.removeHandler(file_handler); file_handler.close()
            continue

        session_trades, session_pnl, session_contracts = run_hourly_session_backtest(
            session_markets_df.sort_values(by='market_close_time_iso'),
            binance_session_features_df,
            global_config_state # Pass the mutable dict
        )
        ...
        logger.info(f"  Trades in Session: {len(session_trades)}") # This is actually decision points logged
        executed_session_trades = sum(1 for t in session_trades if t.get('num_contracts_sim',0) > 0)
        logger.info(f"  Executed Trades in Session: {executed_session_trades}")
        logger.info(f"  Contracts Traded in Session: {session_contracts}")
        logger.info(f"  P&L for Session: ${session_pnl/100:.2f}")
        logger.info(f"  Capital after Session: ${global_config_state['current_capital_usd']:.2f}")
        
        if file_handler: logging.root.removeHandler(file_handler); file_handler.close()
    
    console_handler_final = logging.StreamHandler()
    console_handler_final.setFormatter(formatter) # Use existing formatter
    logging.root.addHandler(console_handler_final)
    
    logger.info(f"\n\n======= OVERALL HISTORICAL BACKTEST SUMMARY (Classifier + Kelly) =======")
    ...
    num_executed_trades_overall = sum(1 for trade in overall_trades_log if trade.get('num_contracts_sim', 0) > 0)
    logger.info(f"Total Decisions Logged: {len(overall_trades_log)}")
    logger.info(f"Total Trades Executed: {num_executed_trades_overall}, Total Contracts: {overall_contracts_traded}")
    logger.info(f"Total P&L: ${overall_pnl_cents/100:.2f}")
    
    if num_executed_trades_overall > 0:
        avg_pnl_per_trade_str = f"${overall_pnl_cents/num_executed_trades_overall/100:.4f}"
        logger.info(f"Avg P&L/Executed Trade: {avg_pnl_per_trade_str}")
    if overall_contracts_traded > 0:
        avg_pnl_per_contract_str = f"${overall_pnl_cents/overall_contracts_traded/100:.4f}"
        logger.info(f"Avg P&L/Contract: {avg_pnl_per_contract_str}")

    if overall_trades_log:
        df_overall_results = pd.DataFrame(overall_trades_log)
        overall_results_csv_path = LOGS_DIR.parent / f"backtest_overall_LogRegKelly_{run_timestamp}.csv"
...
if __name__ == "__main__":
    if strategy._model is None or strategy._scaler is None or strategy._feature_order is None:
        logger.critical("Classifier model artifacts not loaded in strategy. Aborting.")
    elif not MARKET_OUTCOMES_CSV_PATH:
        logger.critical("MARKET_OUTCOMES_CSV_PATH is not set. Aborting.")
    else:
        logger.info("Starting Historical Backtest with Logistic Regression Model and Kelly Sizing...")
        main_backtest_loop()